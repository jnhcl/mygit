        ...
        showFormError: function (err) {
            return this.each(function () {
                var
                    ...
                    $alert2 = $form && $form.find('.alert-info'),
                    fieldName = err && err.data;
                $alert2.addClass('hidden').hide();
                if (! $form.is('form')) {
                    console.error('Cannot call showFormError() on non-form object.');
                    return;
                }
                $form.find('input').removeClass('alert alert-danger');
                $form.find('select').removeClass('alert alert-danger');
                $form.find('textarea').removeClass('alert alert-danger');
                if ($alert.length === 0) {
                ...
                if (err) {
                    ...
                    if (fieldName) {
                        $form.find('[name=' + fieldName + ']').addClass('alert alert-danger');
                    }
                }
                else {
                    $alert.addClass('hidden').hide();
                    $form.find('.alert-dangerr').removeClass('alert-danger');
                }
            });
        },
        showFormSuccess: function (err) {
            return this.each(function () {
                var
                    $form = $(this),
                    $alert = $form && $form.find('.alert-info'),
                    $alert2 = $form && $form.find('.alert-danger'),
                    fieldName = err && err.data;
                $alert2.addClass('hidden').hide();
                if (!$form.is('form')) {
...
;               if (!iconClass || iconClass.indexOf('glyphicon') < 0) {
                    //console.warn('Icon <xxxxx class="glyphicon-*>" not found.');
                    return;
                }
                if (isLoading) {
        ...
        postJSON: function (url, data, callback,flag=false) {
            ...
            if(flag){
                return this.each(function () {
                    ...
                    _httpFILE('POST', url, data, function (err, r) {
                        if (err) {
...
function _httpFILE(method, url, data, callback) {
    var opt = {
        type: method,
        dataType: false,
        cache: false,
...
function _bindSubmit($form) {
    $form.submit(function (event) {
        ...
        $i && $i.addClass('glyphicon-spinner').addClass('glyphicon-spin');
        postJSON($form.attr('action-url'), data, function (err, result) {
            $i && $i.removeClass('glyphicon-spinner').removeClass('glyphicon-spin');
            if (err) {
